...
db.query = jest.fn(async (text, params) => client.query(text, params));
...
describe('webhook-processor worker', () => {
  beforeEach(() => {
    ...
    client.query.mockImplementation((text, params) => {
      ...
      if (typeof text === 'string' && text.includes('SELECT status') && text.includes('FOR UPDATE')) {
        return Promise.resolve({ rowCount: 1, rows: [{ status: 'pending' }] });
      }
  ...
  test('processEvent skips business logic when execution log already succeeded', async () => {
    const event = {
      id: 99,
      event_type: 'payment.captured',
    ...
    client.query.mockImplementation((text, params) => {
      ...
      if (typeof text === 'string' && text.includes('SELECT status') && text.includes('FOR UPDATE')) {
        return Promise.resolve({ rowCount: 1, rows: [{ status: 'success' }] });
      }
      if (typeof text === 'string' && text.includes('UPDATE webhook_events')) {
        return Promise.resolve({ rowCount: 1 });
      }
