...
/**
 * P1-05 FIX: JWT audience + issuer claims.
 * Tokens now include `iss` (issuer) and `aud` (audience) so cross-tenant
 * token reuse is detectable. Defaults are safe but should be overridden
 * via JWT_AUDIENCE and JWT_ISSUER env vars in production.
 */
...
function getJwtSecret() {
  ...
  if (!secret) {
    throw new Error('JWT_SECRET is not configured — refusing to start without a signing key');
  }
  return secret;
}
