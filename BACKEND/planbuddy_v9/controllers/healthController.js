 ...
 */
// ─── P0-04 FIX: /health/detailed — full dependency snapshot ──────────────────
//
// Mounted under /internal in app.js (IP-guarded). Returns DB+pool, Redis,
// production health, backpressure, and event-loop-lag snapshots in one call.
exports.detailed = async (req, res) => {
  ...
  try {
    ...
    snapshot.db = { status: 'down', error: err.message };
  }

  // Redis probe
  try {
    const { isHealthy } = require('../config/redis');
    snapshot.redis = await isHealthy();
  } catch (err) {
    snapshot.redis = { status: 'down', error: err.message };
  }

  // Production health (DLQ / integrity)
  try {
    const ph = require('../services/productionHealth');
    snapshot.productionHealth = ph.getMetricsSnapshot();
  } catch (err) {
    snapshot.productionHealth = { error: err.message };
  }

  // Backpressure
  try {
    const { getBackpressureStatus } = require('../middleware/backpressure');
    snapshot.backpressure = getBackpressureStatus();
  } catch (err) {
    snapshot.backpressure = { error: err.message };
  }

  // Event-loop lag gauge value (best-effort)
  try {
    const monitoring = require('../utils/monitoring');
    const lag = await monitoring.register
      .getSingleMetric('planbuddy_event_loop_lag_seconds')?.get();
    snapshot.eventLoopLag = lag?.values?.[0]?.value ?? null;
  } catch (_) { /* non-fatal */ }

  snapshot.durationMs = Date.now() - started;

  // Return 200 if db is ok, 503 otherwise. Never return 200 with db down.
  const code = snapshot.db?.status === 'ok' ? 200 : 503;
