...
async function applyPaymentEvent(client, { eventType, paymentId, eventId }) {
  ...
  if (lockResult.rows.length === 0) {
    ...
    logger.warn({ eventId, eventType, paymentId }, '[webhook-processor] Payment not found for payment event — retryable');
    throw err;
  }
...
async function applyRefundEvent(client, { eventType, payload, refundId, eventId }) {
  if (!refundId) {
    const err = Object.assign(new Error('Missing refundId for refund event'), {
  ...
  if (!paymentId) {
    const err = Object.assign(new Error('Missing payment dependency for refund event'), {
      code: 'PAYMENT_DEPENDENCY_MISSING',
      status: 409,
    });
    logger.warn({ eventId, eventType, refundId }, '[webhook-processor] Refund event missing payment dependency — retryable');
    throw err;
  }
  ...
  if (lockResult.rows.length === 0) {
    const err = Object.assign(new Error('Payment dependency missing for refund event'), {
      code: 'PAYMENT_NOT_FOUND',
      status: 409,
    });
    logger.warn({ eventId, eventType, refundId, paymentId }, '[webhook-processor] Payment not found for refund event — retryable');
    throw err;
  }
