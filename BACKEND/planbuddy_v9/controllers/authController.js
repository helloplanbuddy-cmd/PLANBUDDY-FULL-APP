...
const { generateToken, revokeToken, revokeAllUserTokens, decodeToken, verifyToken } = require('../utils/jwt');
...
exports.login = async (req, res, next) => {
  try {
    ...
    if (!passwordValid) {
      ...
      const lockUntilValue = newAttempts >= MAX_FAILED_LOGINS
        ? new Date(Date.now() + LOCKOUT_MINUTES * 60 * 1000)
        : null;
      await db.query(
        `UPDATE users SET failed_login_attempts = $1, locked_until = $3 WHERE id = $2`,
        [newAttempts, user.id, lockUntilValue]
      );
  ...
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      try {
        ...
        if (decoded?.jti) {
          await revokeToken(decoded.jti, decoded.sub || decoded.id || null, db, redis, {
            expiresAt: decoded.exp ? new Date(decoded.exp * 1000) : undefined,
          });
        }
      } catch (verifyErr) {
        logger.warn({ requestId: req.requestId, err: verifyErr.message }, '[auth] logout: invalid access token ignored');
      }
    }
